...
async def generateImage(request: Request):
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Please create a propor DALL-E prompt based on the following json message:"+json.dumps(input)
            }
        ])

    prompt_image = completion.choices[0].message.content

    client = AzureOpenAI(
        api_version="2024-02-01",
        azure_endpoint="https://team3openai.openai.azure.com/",
